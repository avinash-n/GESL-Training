			...
			if (i < 768) {
				...
				for (j = 0; j < 1024; j++) {
					...
					if (*(pt_addr + j) != NULL){
						...
						i, (((ptr->mm->pgd) + i)->pgd),
						(unsigned long) (kmap (&mem_map[pgd_phaddr >> 12])));

						printk (KERN_CONT "%3p --> %8p --> %8p --> :: %8p --> %8p\n", j,
						*(pt_addr + j), (unsigned long) (kmap (&mem_map[pt_phaddr >> 12])),
						*(page_phaddr), (unsigned long) (kmap (&mem_map[page_vaaddr >> 12])));
					}
				}
			} /*else if (i >= 768) {
				pgd_phaddr = (unsigned long)((((ptr->mm->pgd) + i)->pgd));
