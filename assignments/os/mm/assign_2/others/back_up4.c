...
int i;				// Loop variable(pgd)
int j;				// Loop variable(pt)
unsigned long pgd_phaddr;		// Physical address in PGD
unsigned long pt_phaddr;		// Physical address in PT
unsigned long page_vaaddr;		// Virtual address of Page frame 
unsigned int *pt_addr;		// Physical address in PT
unsigned int *page_phaddr;		// Physical address of Page frame
...
//										": ------PAGE FRAME------\n");
	...
	for (i = 0; i < 1024; i++) {
		...
		if ((((ptr->mm->pgd) + i)->pgd) != 0x0) {

			if (i < 768) {

				pgd_phaddr = (unsigned long)(((ptr->mm->pgd) + i)->pgd);
				pt_addr = (unsigned int *) kmap (&mem_map[pgd_phaddr >> 12]);

				for (j = 0; j < 1024; j++) {

					if (*(pt_addr + j) != 0x0) {

						pt_phaddr = (unsigned long) (*(pt_addr + j));
						page_phaddr = (unsigned int *) kmap (&mem_map[pt_phaddr >> 12]);
						page_vaaddr = (unsigned long) (*(page_phaddr));	

						printk (KERN_INFO "%3x --> %8x --> %8p :: ",
						i, (((ptr->mm->pgd) + i)->pgd),	(kmap (&mem_map[pgd_phaddr >> 12])));

						printk (KERN_CONT "%3x --> %8x --> %8x --> :: %8x --> %8x\n", j,
						*(pt_addr + j), (unsigned int) (kmap (&mem_map[pt_phaddr >> 12])),
						*(page_phaddr), (unsigned int) (kmap (&mem_map[page_vaaddr >> 12])));
					}
				}
			} /*else if (i >= 768) {
				pgd_phaddr = (unsigned long)((((ptr->mm->pgd) + i)->pgd));
				printk (KERN_INFO "%3p --> %8p --> %8p\n", i,
				(((ptr->mm->pgd) + i)->pgd), (unsigned long) (pgd_phaddr + 0xc0000000));
			}*/
		}
	}
