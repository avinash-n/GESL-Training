...
pte_t *pgd_va_index;		// Virtual address pointed by PGD entry
int i = 0;
...
long myIoctl (struct file *fd, unsigned int pid, unsigned long addr) {
	...
	pgd_ph_index = (unsigned long) ((ptr->mm->pgd + (addr >> 22))->pgd);
		printk (KERN_INFO "PGD entry(ph) --> %8lx\n", pgd_ph_index);
	pgd_va_index = (pte_t *) (kmap (&mem_map[pgd_ph_index >> 12]));
		printk (KERN_INFO "PGD entry(va) --> %8lx\n", (unsigned long int)pgd_va_index);
	
	printk (KERN_INFO "PT entries\n");
	while (i < 1024) {
		if ((pgd_va_index + i) -> pte) {
			printk (KERN_INFO "\t\t\t\t\t%3x --> %8lx\n", i, ((pgd_va_index + i) -> pte));
		}
	i++;
	}
