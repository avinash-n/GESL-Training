...
unsigned long pgd_ph_index;		// Physical address in PGD entry
unsigned long *pgd_va_index;		// Virtual address pointed by PGD entry
unsigned long pt_ph_index;		// Physical address in PT
unsigned long *pt_va_index;		// Virtual address pointed by PT entry
...
long myIoctl (struct file *fd, unsigned int pid, unsigned long addr) {
	...
	pgd_ph_index = (unsigned long) ((ptr->mm->pgd + (addr >> 22))->pgd);
	pgd_va_index = (unsigned long *) (kmap (&mem_map[pgd_ph_index >> 12]));
	
	pt_ph_index = (unsigned long) (*(pgd_va_index + ((addr & 0x003ff000) >> 12)));
	pt_va_index = (unsigned long *) (kmap (&mem_map[pt_ph_index >> 12]));

	printk (KERN_INFO "Page frame base address(ph) --> %x\n", (unsigned int)pt_ph_index);
	printk (KERN_INFO "Page frame base address(va) --> %x\n", (unsigned int)pt_va_index);
