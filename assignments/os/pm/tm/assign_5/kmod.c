...
struct task_struct *pid_ptr1;	// Pointer to task_struct to read pid
struct task_struct *pid_ptr2;	// Dummy pointer to task_struct
...
long myIoctl (struct file *fd, unsigned int pid, unsigned long fork_pid) {
	printk(KERN_INFO "ioctl() function invoked\n");

	pid_ptri2 = find_task_by_vpid(pid);


	if (fork_pid > 0) {
		printk(KERN_INFO "----In parent process----\n");
		printk(KERN_INFO "sys/PID of the current process --> %d\n", pid_ptr1->pid);
	} else if (fork_pid == 0) {
		printk(KERN_INFO "----In child process----\n");
		printk(KERN_INFO "sys/PID of the current process --> %d\n", pid_ptr1->pid);
		list_for_each_entry(pid_ptr2, &(pid_ptr1->tasks), tasks) {
			printk(KERN_INFO "%d\t", pid_ptr2->pid);
			printk(KERN_CONT "%s\t", pid_ptr2->comm);
			printk(KERN_CONT "utime:%u\t", pid_ptr2->prev_cputime.utime);
			printk(KERN_CONT "stime:%u\t", pid_ptr2->prev_cputime.stime);
	} else {
