...
struct task_struct *pid_ptr;	// Pointer to pid in task_struct
struct files_struct *files_ptr;	// Pointer to files_struct in task_struct
struct fdtable *fdt_ptr;	// Pointer to fdtable in files_struct
struct file **fd_ptr;	// Pointer to file in fdtable
...
long myIoctl (struct file *fd, unsigned int pid, unsigned long vfork_pid) {

		printk(KERN_INFO "ioctl() function invoked\n");

		pid_ptr = find_task_by_vpid(pid);

		files_ptr = pid_ptr -> files;
		fdt_ptr = files_ptr -> fdt;
		fd_ptr = fdt_ptr -> fd;
		...
		printk(KERN_INFO "Session descriptor of the current process --> %p\n", *(fd_ptr + 3));
